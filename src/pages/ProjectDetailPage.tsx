                      ...
                      {(project.role || project.timeline || project.year || project.demoStack || project.originalStack) ? (
                        <>
                          ...
                          {project.demoStack ? (
                            <>
                              <span className="hidden text-muted-2 sm:inline" aria-hidden>·</span>
                              <span className="rounded-full bg-white/5 px-2.5 py-1 text-xs font-medium text-muted-2 ring-1 ring-white/10" title="Demo">
                          ...
                          {project.originalStack ? (
                            <>
                              <span className="hidden text-muted-2 sm:inline" aria-hidden>·</span>
                              <span className="rounded-full bg-white/5 px-2.5 py-1 text-xs font-medium text-muted-2 ring-1 ring-white/10" title="Original / Code">
                  ...
                  <div className="flex w-full shrink-0 gap-2 sm:w-auto">
                    {project.demoUrl ? (
                      <a
                        ...
                        className="inline-flex min-w-0 flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-medium text-ink ring-1 ring-white/10 transition-all hover:bg-primary/10 hover:text-primary hover:ring-primary/20 sm:flex-initial"
                      >
                ...
                {project.tags.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2 text-muted-1">
                    ...
                    {project.tags.map((t) => (
                      <span
                        key={t}
                        className="rounded-md border border-primary/35 bg-primary/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-primary"
                      >
                        {t}
                      </span>
                    ))}
        ...
        <Container>
          <Reveal delay={0.05}>
            <GlassCard className="mt-8 md:mt-10" blogStyle accent="left" panelClassName="p-6 pl-5 md:p-10 md:pl-6 lg:p-12 lg:pl-8">
                {project.summary && (
                  <div className="mb-8 rounded-xl border-l-4 border-primary bg-primary/10 py-4 pl-5 pr-5">
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      pre: (props) => (
                        <pre
                          {...props}
                          className="overflow-x-auto rounded-xl border border-white/10 border-l-4 border-l-primary bg-white/5 py-4 pl-4 shadow-inner"
                        />
                      ),
                      code: (props) => {
                        ...
                        if (isInline)
                          return (
                            <code
                              {...props}
                              className="rounded bg-white/10 px-1.5 py-0.5 text-sm font-medium text-ink"
                            />
                          );
                        return <code {...props} />;
                      },
